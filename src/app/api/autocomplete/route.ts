...
export async function GET(req: NextRequest) {
  try {
    ...
    async function fetchGeoapify() {
      ...
      const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(q)}&limit=7&format=json&apiKey=${geoKey}`;
    ...
    async function fetchLocationIQ() {
      if (!locIqKey) throw new Error('Missing LOCATIONIQ_KEY');
      const url = `https://us1.locationiq.com/v1/autocomplete?key=${locIqKey}&q=${encodeURIComponent(q)}&limit=7&tag=place,address`;
      const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
      if (!res.ok) throw new Error(`LocationIQ failed: ${res.status}`);
      const data = await res.json();
      const suggestions = (Array.isArray(data) ? data : []).map((it: any) => ({
